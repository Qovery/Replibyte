...
fn test_memory_allocation_patterns() {
    ...
    assert!(duration_capacity <= duration_new * 2, "with_capacity should not be significantly slower");
}
...
fn test_buffer_operations() {
    ...
    let duration = start.elapsed();
    println!("Chunked {} bytes into {} chunks in {:?}", test_data.len(), chunks.len(), duration);
...
fn test_concurrent_operations() {
    ...
    let handles: Vec<_> = (0..num_threads).map(|thread_id| {
        thread::spawn(move || {
            ...
            for i in 0..operations_per_thread {
                let query = format!("INSERT INTO table_{} (id, value) VALUES ({}, 'test');", thread_id, i);
    ...
    let duration = start.elapsed();
    println!("Processed {} operations across {} threads in {:?}", 
             num_threads * operations_per_thread, num_threads, duration);
    
    assert!(total > 0);
    assert!(duration.as_secs() < 5, "Concurrent operations should complete quickly");
}
...
fn test_error_handling() {
    ...
    let result = list_sql_queries_from_dump_reader(reader, |_query| {
        ListQueryResult::Continue
    });
