...
mod tests {
    ...
    fn test_column_extraction_performance() {
        ...
        let start = Instant::now();
        for _ in 0..1000 {
            let _columns = mysql_parser
                .extract_insert_columns_fast(mysql_query)
                .unwrap();
        }
        let mysql_duration = start.elapsed();

        println!(
            "PostgreSQL column extraction: 1000 iterations in {:?}",
            pg_duration
        );
        println!(
            "MySQL column extraction: 1000 iterations in {:?}",
            mysql_duration
        );

        assert!(
            pg_duration.as_millis() < 500,
            "PostgreSQL column extraction took too long: {:?}",
            pg_duration
        );
        assert!(
            mysql_duration.as_millis() < 500,
            "MySQL column extraction took too long: {:?}",
            mysql_duration
        );
    }
    ...
    fn test_simd_operations() {
        ...
        println!(
            "SIMD pattern finding: 10000 iterations in {:?}",
            find_duration
        );
        println!(
            "SIMD whitespace skipping: 10000 iterations in {:?}",
            ws_duration
        );

        assert!(
            find_duration.as_millis() < 100,
            "SIMD pattern finding took too long: {:?}",
            find_duration
        );
        assert!(
            ws_duration.as_millis() < 50,
            "SIMD whitespace skipping took too long: {:?}",
            ws_duration
        );
    }
    ...
    fn test_memory_allocation_patterns() {
        ...
        println!(
            "Parser reuse: {} iterations in {:?}",
            iterations, reuse_duration
        );
        println!(
            "New parser each time: {} iterations in {:?}",
            iterations, new_duration
        );

        // Reusing should be faster than creating new instances
        assert!(
            reuse_duration < new_duration,
            "Parser reuse should be faster: {:?} vs {:?}",
            reuse_duration,
            new_duration
        );
    }
