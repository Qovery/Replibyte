...
fn benchmark_postgres_parsing(queries: &[&str]) {
    ...
    for (i, query) in queries.iter().enumerate() {
        ...
        let throughput_original =
            (query_size as f64 * iterations as f64) / original_duration.as_secs_f64() / 1_000_000.0; // MB/s
        let throughput_optimized = (query_size as f64 * iterations as f64)
            / optimized_duration.as_secs_f64()
            / 1_000_000.0; // MB/s

        println!("Query {} ({} bytes):", i + 1, query_size);
        println!(
            "  Original:  {:>8.2}ms ({:>6.1} MB/s)",
            original_duration.as_secs_f64() * 1000.0,
            throughput_original
        );
        println!(
            "  Optimized: {:>8.2}ms ({:>6.1} MB/s)",
